...
export default ({active, data, handleClose, invoiceID, handleUpdate}: PropsType) => {
    ...
    let maxDistance: {fullName: string; distance: number} | number = Object.values(data).filter(
    ...
    const submit = async () => {
        ...
        setErrors(errors);
        if (Object.values(errors).length != 0 || !values.distance || !values.name) return;
        setLoading(true);
        const res = await sendPostRequest(`invoices/assign`, {
