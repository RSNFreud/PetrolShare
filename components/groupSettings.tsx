...
export default ({handleComplete, hideCancel, handleCancel, newGroup}: PropsType) => {
    ...
    const createGroup = async () => {
        ...
        setLoading(true);
        const res = await sendPostRequest('group/create', {
            authenticationKey: retrieveData?.authenticationKey,
            groupID,
        });
    ...
    const updateGroup = async (groupID?: string, message?: string) => {
        ...
        const res = await sendPostRequest('group/update', {
            authenticationKey: retrieveData?.authenticationKey,
        ...
        if (res?.ok) {
            setLoading(false);
            if (!newGroup) sendCustomEvent('sendAlert', 'Group settings successfully updated!');
            ...
            <>
                ...
                {!newGroup && (
                    ...
                    >
                        ...
                        >
                            By changing group settings you will reset your current tracked session.
                        </Text>
                    </Box>
                )}
                <Text
