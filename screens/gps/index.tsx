...
export default () => {
    ...
    const calculateDistance = async (latitude: number, longitude: number) => {
        ...
        if (!oldData) return await setItem("gpsOldData", JSON.stringify({ latitude: latitude, longitude: longitude }))
        const currDistance: { longitude: number; latitude: number } = JSON.parse(oldData)
        ...
        await setItem("gpsOldData", JSON.stringify({ latitude: latitude, longitude: longitude }));
    ...
    const startTracking = async () => {
        ...
        const trackNumber = Geolocation.watchPosition(success => {
            ...
            interval: 300, enableHighAccuracy: true, distanceFilter: 5, maximumAge: 0, timeout: 1000
        })
