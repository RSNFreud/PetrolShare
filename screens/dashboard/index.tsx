...
export default () => {
    ...
    const checkForReferral = async () => {
        ...
        if (params) {
            const groupID = params['groupID']?.toString().split('groupID=')[1]?.replace(':', '');
            if (groupID) {
                sendReferal(groupID);
            }
        }
    ...
    const changeGroup = async (groupID: string) => {
        const res = await sendPostRequest(`user/change-group`, {
    ...
    const checkForUnconfirmedDistance = async () => {
        try {
            const res = await sendRequestToBackend({
                url: `distance/check-distance?authenticationKey=${retrieveData?.authenticationKey}`,
            });
    ...
    const renderPopupContent = () => {
        switch (currentScreen) {
            case 'Settings':
                return <GroupSettings handleComplete={handleClose} newGroup hideCancel />;
            case 'ConfirmDistance':
                if (!confirmDistanceData) return <></>;
                return <ConfirmDistance handleComplete={handleClose} {...confirmDistanceData} />;
